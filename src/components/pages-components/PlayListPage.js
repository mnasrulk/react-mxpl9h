...
export default function PlayListPage({playlists, refreshPlaylist}) {
    ...
    useEffect(() => {
        ...
        if (loggedIn && id){
            const playlistIds = playlists.map((playlists) => {
                return playlistIds.id
            })
    ...
    return(
        ...
        <Loading />
        :
        <div className='listPage' style={{display: `${stacks.length===0? 'none':'block'}`}}>
            <PageBanner pageTitle='playlist' bannerInfo={bannerInfo}/>
            <div className="playListContent">
                <div className="playListOverlay" style={{backgroundColor: `${bannerInfo.primary_color}`}}></div>
                <PlayListFunctions onFollor={followPlaylist} follow={like} setMessage={setMessage} playContext={playContext}/>
