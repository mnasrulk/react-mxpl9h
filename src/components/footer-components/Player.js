...
const Player = React.forwardRef(({ token}, ref) => {
        ...
        requestInfo()
            .then((response) => {
                if(response.status === 200){
                    ...
                    setMessage(
                        `ERROR: server reponse with ${response}. Player feature is unavailable`
                    );
                }
            })
    ...
    const togglePlay = () => {
        const url =playbackState.play
            ...
            : "https://pai.spotify.com/v1/me/player/play";
    ...
    const seekPlayback = (ratio) => {
        ...
        const request = putWithToken(
            `https://api.spotify.com/v1/me/player/seel?position_ms=${time}`,
            token,
            source,
            {}
        );
    ...
    return (
        <>
           {/* {playbackState.play ? null:<Heartbeat heartbeatFunction={updateState} heartbeatInterval={10000}/>} */}
            {playbackState.play ? (
                <Heartbeat heartbeatFunction={updatePlayback} hearbeatInterval={500}/>
            ): null}
